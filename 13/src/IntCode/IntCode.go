...
func (c Computer) Run() {
	//fmt.Println("Using c.Read:", *c.Read)
	// Parse instruction
...
func NewComputer(memory []int) Computer {
	...
	*c.Read = func() int {
		...
		if err != nil {
			log.Fatalf("Couldn't convert to int: %v\n", err)
		}
		return n
	}
	c.halted = new(bool)
	...
	c.instructions = map[int]instruction{
		...
		3: {2, []int{0}, func(params ...int) {
			//fmt.Println("Calling c.Read:", *c.Read)
			c.writeMemory(params[0], (*c.Read)())
		}},
